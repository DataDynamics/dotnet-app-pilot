...
public sealed class ReflectionUtils
{
    ...
    /// </returns>
    public static MethodInfo MapInterfaceMethodToImplementationIfNecessary(MethodInfo methodInfo,
    ...
    public static Type[] GetParameterTypes(ParameterInfo[] args)
    {
        ...
        if (args.Length == 0) return EmptyTypes;

        var types = new Type[args.Length];
        for (var i = 0; i < (uint)args.Length; i++) types[i] = args[i].ParameterType;
    ...
    public static string[] GetGenericParameterNames(Type[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++) names[i] = args[i].Name;

        return names;
    }

    public static MethodInfo GetGenericMethod(Type type, string methodName, Type[] typeArguments,
        Type[] parameterTypes)
    {
            ...
            try
            {
                ...
                if (parameters.Length != paramValues.Length)
                    ...
                    for (var i = 0; i < parameters.Length; i++)
                    {
                        ...
                        if (paramValue == null || paramType != paramValue.GetType()) isExactMatch = false;
                    }
            }
            ...
            if (isMatch)
            {
                ...
                if (matchCount == 1)
                    ...
                    throw new AmbiguousMatchException(
                        string.Format(
                            "Ambiguous match for {0} '{1}' for the specified number and types of arguments.",
                            methodTypeName,
                            m.Name));
            }
        }
    ...
    public static object[] PackageParamArray(object[] argValues, int argCount, Type elementType)
    {
        ...
        var j = 0;
        while (i < argValues.Length) paramArray.SetValue(argValues[i++], j++);
    ...
    public static object GetDefaultValue(Type type)
    {
        ...
        if (type == typeof(char)) return char.MinValue;
        ...
        if (type.IsEnum)
        {
            ...
            if (values == null || values.Length == 0)
                throw new ArgumentException(
                    "Bad 'enum' Type : cannot get default value because 'enum' has no values.");
    ...
    public static object[] GetDefaultValues(Type[] types)
    {
        ...
        for (var i = 0; i < types.Length; ++i) defaults[i] = GetDefaultValue(types[i]);
        ...
        MethodInfo candidate, Type[] parameterTypes)
    {
        ...
        if (candidatesParameterTypes.Length != parameterTypes.Length) return false;
    ...
    {
        if (type == null || StringUtils.IsNullOrEmpty(name)) return false;
    ...
    private static object ConvertConstructorArgsToObjectArrayIfNecessary(object value)
    {
        ...
        for (var i = 0; i < constructorArguments.Count; i++) arguments[i] = constructorArguments[i].Value;
    ...
    public static MethodInfo[] GetMatchingMethods(Type type, MethodInfo[] methods, bool strict)
    {
        ...
        for (var i = 0; i < methods.Length; i++)
        {
            ...
            var match = type.GetMethod(method.Name, flags, null, GetParameterTypes(method),
                null);
    ...
    {
        if (SystemUtils.MonoRuntime) return ex.InnerException;
    ...
    public static object GetInstanceFieldValue(object obj, string fieldName)
    {
        ...
        throw new ArgumentException(string.Format(
            "Non-public instance field '{0}' could not be found in class of type '{1}'", fieldName,
            obj.GetType()));
    }
            ...
            if (f.FieldType != fieldValue.GetType())
                throw new ArgumentException(
                    string.Format(
                        ...
                        fieldName, obj.GetType(), f.FieldType,
    ...
    private static MemberwiseCopyHandler GetImpl(Type type)
    {
        ...
        if (s_handlerCache.TryGetValue(type, out handler)) return handler;
