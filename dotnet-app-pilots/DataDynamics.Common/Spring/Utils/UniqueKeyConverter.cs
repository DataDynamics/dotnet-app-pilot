...
public class UniqueKeyConverter : TypeConverter
{
    ...
    public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
    {
        ...
        if (!CanConvertFrom(context, value.GetType()))
            throw new NotSupportedException(string.Format("Conversion from value of type '{0}' is not supported",
                value.GetType().FullName));

        if (value is string)
            return new UniqueKey(value as string);
        if (value is UniqueKey) return value;

        throw new NotSupportedException(string.Format("Cannot convert from value of type '{0}' to '{1}'",
            value.GetType().FullName, typeof(UniqueKey).FullName));
    }
        ...
        Type destinationType)
    {
        ...
        if (!CanConvertTo(context, destinationType))
            throw new NotSupportedException(string.Format("conversion to destinationType '{0}' is not supported.",
                destinationType.FullName));

        // value must either be null or a UniqueKey
        if (value != null && typeof(UniqueKey) != value.GetType())
            throw new NotSupportedException(string.Format("value is not of type '{0}'",
                typeof(UniqueKey).FullName));

        if (value == null) return null;

        if (typeof(string) == destinationType)
            return value.ToString();
        if (typeof(UniqueKey) == destinationType) return value;

        throw new NotSupportedException(string.Format("destinationType must be '{0}' but was '{1}'",
            typeof(string).FullName, destinationType));
    }
}