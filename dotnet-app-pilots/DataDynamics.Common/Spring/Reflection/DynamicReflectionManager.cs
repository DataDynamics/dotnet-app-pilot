...
public sealed class DynamicReflectionManager
{
    ...
    public static PropertySetterDelegate CreatePropertySetter(PropertyInfo propertyInfo)
    {
        ...
        Type[] argumentTypes = { typeof(object), typeof(object), typeof(object[]) };
    ...
    public static object ConvertValueTypeArgumentIfNecessary(object value, Type targetType, int argIndex)
    {
        if (value == null)
        {
            if (ReflectionUtils.IsNullableType(targetType)) return null;
            throw new InvalidCastException(string.Format("Cannot convert NULL at position {0} to argument type {1}",
                argIndex, targetType.FullName));
        }
        ...
        if (!valueType.IsValueType)
            ...
            throw new InvalidCastException(string.Format(
                "Cannot convert value '{0}' of type {1} at position {2} to argument type {3}", value,
...
//                object v = fieldInfo.GetValue(null); // ensure type is initialized...
    ...
    internal static void EmitPropertySetter(ILGenerator il, PropertyInfo propertyInfo, bool isInstanceMethod)
    {
            ...
            && !(propertyInfo.DeclaringType.IsValueType && !method.IsStatic))
        {
            ...
            for (var i = 0; i < args.Length; i++) SetupOutputArgument(il, paramsArrayPosition, args[i], outArgs);
            ...
            for (var i = 0; i < args.Length; i++) ProcessOutputArgument(il, paramsArrayPosition, args[i], outArgs);
            il.Emit(OpCodes.Ret);
        }
        else
        {
    ...
    internal static void EmitInvokeConstructor(ILGenerator il, ConstructorInfo constructor, bool isInstanceMethod)
    {
        ...
        for (var i = 0; i < args.Length; i++) SetupMethodArgument(il, paramsArrayPosition, args[i], null);
